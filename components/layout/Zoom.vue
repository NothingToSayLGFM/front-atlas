...
export default {
  ...
  computed: {
    scale: {
      get () {
        return this.$route.path === '/visualisation/map' ? this.$store.state.scale.map : this.$store.state.scale.graph
      },
      set (val) {
        this.$route.path === '/visualisation/map' ? this.$store.commit('SET_MAP_SCALE', val) : this.$store.commit('SET_GRAPH_SCALE', val)
      }
    }
  },
  methods: {
