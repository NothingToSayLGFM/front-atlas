<template>
  ...
  >
    ...
    <template v-if="(meta.countries.total > 3 || meta.organisations.total > 3) && searchLimit" v-slot:append-item>
      <v-divider />
      <v-list-item @click="searchLimit=false">
        <v-list-item-content>
          ...
          <v-list-item-subtitle>
            <span>
              {{ meta.organisations.total }} Organizations found. {{ meta.countries.total }} Countries found.
            </span>
          </v-list-item-subtitle>
        </v-list-item-content>
      </v-list-item>
    ...
    <template v-slot:item="data">
      ...
      <template v-else>
        <v-list-item-avatar>
          <div v-if="!data.item.edate" class="oval" />
          <template v-else>
...
export default {
  ...
  computed: {
    ...
    items () {
      if (this.organisations.length === 0 && this.countries.length === 0 && this.$store.state.selectedItemMap.length === 0) {
        return []
      }
      return [
        { header: 'Countries' },
        ...this.countries,
        ...this.$store.state.selectedItemMap.filter(el => !el.edate),
        { divider: true },
  ...
  methods: {
    clearSelectedItem () {
      this.$store.dispatch('clearSelectedItemMap')
    },
    getColor (item) {
    ...
    getData: _.debounce(function () {
      ...
      if (this.search && this.search.length >= 3) {
        ...
        const obj = {
          ...
          sections: ['organisations', 'countries']
        }
          this.$api.countries.searchMapCountryOrganisations(obj)
          .then((res) => {
